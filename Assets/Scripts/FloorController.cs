...
public class FloorController : MonoBehaviour
{
    ...
    void Update()
    {
        if(IsPlayerInFloor() && player.isBackwards)
        {
            camera.GetComponent<Animator>().Play("camera-back");
            player.isBackwards = false;
        }
    }

    private bool IsPlayerInFloor()
    {
        Vector2 center = boxCollider2D.bounds.center + new Vector3(0f, 0.2f, 0f); ;
        Vector2 size = boxCollider2D.bounds.size + new Vector3(0f, -0.1f, 0f);
